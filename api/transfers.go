...
func (server *Server) createTransfer(ctx *gin.Context) {
	...
	if fromAccount.Owner != authPayload.Username {
		err := errors.New("from account's is not belong to authenticated user")
...
func (server *Server) validAccount(ctx *gin.Context, accountID int64, currency string) (db.Account, bool) {
	...
	if account.Currency != currency {
		err := fmt.Errorf("invalid account [%d] currency %s vs %s", account.ID, account.Currency, currency)
		ctx.JSON(http.StatusInternalServerError, errorResp(err))
		return account, false
	}
